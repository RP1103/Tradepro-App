...
// async function fetchNewsUrls(mainUrl, keywords) {
//     try {
...
// async function summarizeMultipleNews(keywords) {
//     const newsUrls = await fetchNewsUrls(MAIN_URL, keywords);
...
//         await summarizeNewsFromURL(newsData);
//     }
// }
...
// Configuration
const API_KEY = "AIzaSyDfSiFTrnlnuQByZ30BQrGF-w5r4MfIPCU"; // Replace with your actual Google API Key
const MAIN_URL = "https://www.investing.com/news/forex-news"; // URL for Forex news

// Keywords for Forex news
const forexKeywords = ["forex", "currency", "exchange", "fx"];

// Function to fetch news URLs from a given URL for specific keywords
async function fetchNewsUrls(mainUrl, keywords) {
    try {
        console.log("Fetching news URLs from:", mainUrl);
        const response = await axios.get(mainUrl);
...
async function summarizeForexNews() {
    const newsUrls = await fetchNewsUrls(MAIN_URL, forexKeywords);
    if (newsUrls.length === 0) {
        console.log("No relevant Forex news URLs found.");
        return;
    }

    console.log("Found", newsUrls.length, "relevant Forex news URLs.");
    for (const newsData of newsUrls) {
        await summarizeNewsFromURL(newsData);
    }
}

// Call the function to summarize Forex news
summarizeForexNews();
